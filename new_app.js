...
const CHAIN_RPC_NETWORK = "bnb-mainnet";
const CHAIN_PRICE_NETWORK = "bnb-mainnet";
const STANDARD_POSITION_MANAGER_ADDRESS = "0x46a15b0b27311cedf172ab29e4f4766fbe7f4364";
const BSC_CONSTANTS_PATH = "./bsc_vfat_constants.json";
const ENABLE_AERODROME_V2_PATHS = false;
...
async function loadBscConstantsConfig() {
  try {
    ...
    if (configuredFactories.length) {
      SICKLE_FACTORY_ALLOWLIST = configuredFactories;
    }

    const configuredImplementations = normalizeAddressList(payload?.vfatImplementationAllowlist || []);
    if (configuredImplementations.length) {
      VFAT_IMPLEMENTATION_ALLOWLIST = new Set(configuredImplementations.map((value) => value.toLowerCase()));
    }

    const configuredManagers = normalizePositionManagers(payload?.clPositionManagers || []);
    if (configuredManagers.length) {
      CL_POSITION_MANAGERS = configuredManagers;
      rebuildClProtocolByManager();
    }
  } catch {
    // Use fallback in-file constants when JSON config is unavailable.
  }
}
...
const STORAGE_KEYS = {
  ...
  vfatContractsCache: "defi_scanner_bsc_vfat_contract_cache_v1"
};
...
function readCachedVfatContracts(wallet) {
  try {
    ...
    const raw = localStorage.getItem(STORAGE_KEYS.vfatContractsCache);
    if (!raw) {
      return [];
    }
...
function writeCachedVfatContracts(wallet, addresses) {
  try {
    ...
    localStorage.setItem(STORAGE_KEYS.vfatContractsCache, JSON.stringify(parsed));
  } catch {
    // Ignore localStorage access errors.
  }
}
...
function getProvider(apiKey) {
  if (!state.provider || state.providerKey !== apiKey) {
    state.provider = new ethers.JsonRpcProvider(`https://${CHAIN_RPC_NETWORK}.g.alchemy.com/v2/${apiKey}`, BSC_CHAIN_ID, {
      staticNetwork: true
    });
...
async function fetchVFatClDataForWallet(wallet, apiKey, provider, onStatus = () => {}) {
  ...
  if (cachedVfatContracts.length) {
    usedCachedVfatContracts = true;
    onStatus(`Using cached VFat contract set for ${CHAIN_NAME} to reduce RPC load...`);
  ...
  if (usedCachedVfatContracts && !vfatClHistoryRows.length) {
...
async function fetchPortfolio(wallet, apiKey, onStatus = () => {}) {
  ...
  const rawPositions = await Promise.all(
    tokenRefs.map((item) => fetchPosition(item.tokenId, item.managerAddress, provider))
  );
...
function renderSummary(portfolio) {
  ...
  els.walletSubline.textContent = `${totals.openCount} active position${totals.openCount === 1 ? "" : "s"} on ${CHAIN_NAME}`;
  ...
  if (totals.openCount) {
    ...
    setStatus(`No active ${CHAIN_NAME} LP positions found.`);
  }
}
...
async function runLookup({ openSettingsOnMissing = false } = {}) {
  ...
  let didRender = false;
  try {
    setBusy(true, `Reading ${CHAIN_NAME} contracts...`);
...
async function boot() {
  await loadBscConstantsConfig();
  clearDashboard();
  loadPersistedCredentials();
  syncCredentialBanner();
  setStatus(`Loading ${CHAIN_NAME} positions...`);
