...
async function runLookup({ openSettingsOnMissing = false } = {}) {
  ...
  try {
    ...
    closeSettings();
    pushTrace("rendered_open_rows", {
      rows: (portfolio?.openRows || []).map((row) => ({
        chain: row.chainKey,
        protocol: row.protocol,
        source: row.source,
        tokenContract: row.tokenContract,
        ...
        poolFee: row.poolFee,
        currentPoolUsd: row.currentPoolUsd,
        fees24hUsd: row.fees24hUsd,
        emissions24hUsd: row.emissions24hUsd,
        claimableNowUsd: row.vfatClaimableNowUsd
      })),
      incomeDailyUsd: (portfolio?.openRows || []).reduce((sum, row) => (
        sum
        + (Number.isFinite(row?.fees24hUsd) ? row.fees24hUsd : 0)
        + (Number.isFinite(row?.emissions24hUsd) ? row.emissions24hUsd : 0)
      ), 0)
    });
